...
public class DatabaseSync {
    ...
    public void startSyncTask() {
        ...
        if (plugin.isHubMode()) {
            ...
            // smp: 起動時に現在の出品/オーダーをDBへ一括反映 (初回移行/再起動時のズレ補正)
            manager.runGlobalTaskLater(() -> {
    ...
    /** (hub) DBからオークション/オーダー一覧を取得して表示用キャッシュへ反映する */
            ...
            try (PreparedStatement ps = conn.prepareStatement("SELECT * FROM auction_items");
                ...
                while (rs.next()) {
                    try {
                        ...
                        if (ai.item != null) newItems.put(ai.id, ai);
            ...
            try (PreparedStatement ps = conn.prepareStatement("SELECT * FROM order_items");
                ...
                while (rs.next()) {
                    try {
                        ...
                        if (oi.itemTemplate != null) newOrders.put(oi.id, oi);
                    } catch (Exception ignored) {}
                }
            }
            ...
            try (PreparedStatement ps = conn.prepareStatement("SELECT * FROM order_storage");
                ...
                while (rs.next()) {
                    try {
                        ...
                        if (items != null) {
                            newStorage.computeIfAbsent(orderId, k -> new ArrayList<>()).addAll(Arrays.asList(items));
                        }
                    } catch (Exception ignored) {}
                }
            }
